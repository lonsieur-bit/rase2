...
// Serve static files - Hostinger expects files in htdocs
...
app.get('*', (req, res) => {
  ...
  res.sendFile(indexPath, (err) => {
    if (err) {
      // Fallback to basic index.html if dist doesn't exist
      res.sendFile(path.join(__dirname, 'index.html'), (fallbackErr) => {
        if (fallbackErr) {
          res.status(500).send('Server Error: Cannot serve website files');
        }
      });
    }
  });
